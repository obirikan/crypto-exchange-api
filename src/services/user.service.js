...
class UserService {
    ...
    static async signUp(rawData) {
        ...
        if (!rawData.email || !rawData.password || !rawData.confirm_password) {
            return { response: false, message: "Missing required fields!", data: null };
        }
    
        if (rawData.password !== rawData.confirm_password) {
            return { response: false, message: "Passwords do not match!", data: null };
        }
        ...
        if (checker) {
            return { response: false, message: "This Email is already in use", data: null };
        }
        ...
        if (!verifier || verifier.verify_code !== rawData.email_verify) {
            return { response: false, message: "Email verification code is incorrect.", data: null };
        }
    
        // Check invite code
        if (rawData.invite_code && rawData.invite_code !== "0") {
            let inviter = await UserModel.findOne({ id: rawData.invite_code });
            if (!inviter) {
                return { response: false, message: "Invalid invite code", data: null };
            }
        }
        ...
        try {
            await UserModel.create(registerData);
        } catch (error) {
            console.error("User registration error:", error);
            return { response: false, message: "An error occurred during registration.", data: null };
        }
        // await UserModel.create(registerData);
        await ManageUserModel.create({ email: registerData.email, have_wallet: false, wallet_address: "" });
    
        // Send confirmation email
        const subject = i18n.__({ phrase: "MGL Exchange: Account Created Successfully", locale: rawData.locale || "En" });
        const body = i18n.__({ phrase: "Your MGL Exchange account has been created successfully. Thank you.", locale: rawData.locale || "En" });
    ...
    static async userLogin(rawData) {
        ...
        if (!email || !pass) {
            return {response: false, message: "Missing email or password!", data: null};
        }
        ...
        if (!user.password) {
            return {response: false, message: "User password not found!", data: null};
        }
        ...
        if (!secretKey) {
            return {response: false, message: "Server error: Missing JWT secret!", data: null};
        }
        ...
        for (let i = 0; i < wallets.length; i++) {
            if (!wallets[i].privatekey) {
                console.error(`Wallet ${i} has no private key!`);
                continue;
            }
        ...
        return {response: true, message: "Success!", data: { userInfo, token, keyPair }};
    };
